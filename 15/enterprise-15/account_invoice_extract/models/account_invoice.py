...
class AccountMove(models.Model):
    ...
    def _post(self, soft=True):
        ...
        invoices_to_validate = posted.filtered(
            lambda move: move.is_invoice() and move.extract_state == 'waiting_validation'
        )
        if invoices_to_validate:
            account = self._get_iap_account()
            for record in invoices_to_validate:
                values = {
    ...
    def _save_form(self, ocr_results, no_ref=False):
        ...
        with self.get_form_context_manager() as move_form:
            ...
            if not move_form.partner_id:
                ...
                move_form.fiscal_position_id = move_form.partner_id.property_account_position_id
